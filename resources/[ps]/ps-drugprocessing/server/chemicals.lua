...
RegisterServerEvent('ps-drugprocessing:processHydrochloric_acid', function()
	...
	if Player.Functions.RemoveItem('hydrochloric_acid', 1) then
		if Player.Functions.AddItem('chemicals', 1) then
			TriggerClientEvent("inventory:client:ItemBox", source, QBCore.Shared.Items['hydrochloric_acid'], "remove")
...
RegisterServerEvent('ps-drugprocessing:processsodium_hydroxide', function()
	...
	if Player.Functions.RemoveItem('sodium_hydroxide', 1) then
		if Player.Functions.AddItem('chemicals', 1) then
			TriggerClientEvent("inventory:client:ItemBox", source, QBCore.Shared.Items['sodium_hydroxide'], "remove")
			TriggerClientEvent("inventory:client:ItemBox", source, QBCore.Shared.Items['chemicals'], "add")
...
RegisterServerEvent('ps-drugprocessing:processprocess_sulfuric_acid', function()
	...
	if Player.Functions.RemoveItem('sulfuric_acid', 1) then
		if Player.Functions.AddItem('chemicals', 1) then
			TriggerClientEvent("inventory:client:ItemBox", source, QBCore.Shared.Items['sulfuric_acid'], "remove")
			TriggerClientEvent("inventory:client:ItemBox", source, QBCore.Shared.Items['chemicals'], "add")
