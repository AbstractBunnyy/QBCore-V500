...
## Setup for [lj-inventory](https://github.com/overextended/lj-inventory)

* Find `lj-inventory:buyItem` on modules > shops> server.lua
...
if string.find(fromData.name, "WEAPON_") then
					...
					local imageurl = ("https://cfx-nui-lj-inventory/web/images/%s.png"):format(fromData.name)
